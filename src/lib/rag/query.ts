...
import { getRagIndex, claimBuild, markReady, markError } from "./db";
import {
...
export async function queryRag(input: QueryRagInput): Promise<RagQueryOutcome> {
  ...
  // Cost ledger: one row for the query embed, one for the Modal compute. Deterministic keys.
...
export async function buildOrRefreshIndex(input: {
  ...
  const claim = await claimBuild(input.ownerId, zonePath, embeddingModel);
