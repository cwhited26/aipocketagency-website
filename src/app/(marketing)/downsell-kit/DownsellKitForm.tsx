...
// The secondary downsell: the $97 AI Office DIY Setup Kit. Bought by a fresh visitor (no login), so it
// collects an email and routes to the addon-checkout with kind=diy_setup_kit. On success the webhook
// emails a signed download link.
export default function DownsellKitForm({ defaultEmail = "" }: { defaultEmail?: string }) {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/pocket-agent/addon-checkout", {
        ...
        body: JSON.stringify({ kind: "diy_setup_kit", email }),
      });
      if (!res.ok) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      >
        {busy ? "Loading…" : "Get the DIY Setup Kit — $97"}
      </button>
    </form>
  );
}
