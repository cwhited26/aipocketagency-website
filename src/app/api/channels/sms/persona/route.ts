// POST /api/channels/sms/persona — set the Persona that answers on the owner's SMS channel
// (PA-CHAN-8). Body: { personaId: string | null }. Mirrors the Telegram route.
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const res = await setChannelConnectionPersona(user.id, "sms", personaId);
