// POST /api/channels/imessage/disconnect — remove the owner's iMessage channel connection
// (PA-CHAN-1). Deletes the pa_channel_connections row; pa_channel_messages cascade. There's no
// provider-side webhook to tear down (the owner registered it in their own BlueBubbles server; the
...
export async function POST(): Promise<NextResponse> {
  ...
  const res = await deleteChannelConnection(user.id, "imessage");
