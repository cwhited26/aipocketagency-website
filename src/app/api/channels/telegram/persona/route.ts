// POST /api/channels/telegram/persona — set the Persona that answers on the owner's Telegram bot
// (PA-CHAN-8). Body: { personaId: string | null }. A non-null id is verified to belong to the caller
// before it's stored. null clears it back to the gateway's default Persona. Mirrors the Slack route.
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const res = await setChannelConnectionPersona(user.id, "telegram", personaId);
