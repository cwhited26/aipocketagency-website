...
export default async function BrainInboxPage() {
  ...
  if (hasBrain && paUser.brain_repo && ghToken) {
    ...
    // Three capture pipelines land items in different places:
    ...
    const voiceEntries = (
      await Promise.all(
        voiceFiles.map(async (f) =>
          parseVoiceMemoFile(f.path, await fetchFileContent(repo, f.path, ghToken)),
        ),
      )
    ).filter((e): e is NonNullable<typeof e> => e !== null);

    entries = mergeInboxEntries(blockEntries, shareEntries, voiceEntries);
  }
