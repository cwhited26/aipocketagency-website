...
function RecallIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 16 16" fill="none" aria-hidden="true">
      <rect x="1.5" y="3.5" width="9" height="9" rx="2" stroke="currentColor" strokeWidth="1.1" />
      <path d="M11 6.5l3.5-2v7L11 9.5" stroke="currentColor" strokeWidth="1.1" strokeLinejoin="round" />
    </svg>
  );
}
...
export default function RecallAiConnectCard({
  connected,
  verifiedAt,
}: {
  connected: boolean;
  ...
  async function handleConnect() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/app/meeting-persona/connect", {
        method: "POST",
    ...
    <div className="rounded-xl border border-slate-700/60 bg-slate-900/50 px-5 py-5">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold text-slate-100">Recall.ai</p>
            {connected ? (
              <p className="text-sm text-slate-400 mt-0.5 leading-relaxed">
                Connected{verified ? ` — key checked ${verified}` : ""}. This is the meeting-bot
                service your Meeting Persona uses to join Zoom, Google Meet, and Teams calls.
              </p>
            ) : (
              <p className="text-sm text-slate-500 mt-0.5 leading-relaxed">
                Recall.ai is the meeting bot service your Meeting Persona uses to join Zoom, Google
                Meet, and Teams calls. Sign up at recall.ai, paste your API key, you&apos;re connected.
              </p>
            )}
          </div>
        </div>
      ...
      {!connected && (
        <div className="mt-4 pl-7 flex flex-col gap-3">
          ...
          >
            {connecting ? "Connecting…" : "Connect Recall.ai →"}
          </button>
        </div>
      )}
